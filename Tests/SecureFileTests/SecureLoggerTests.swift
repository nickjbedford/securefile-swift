...
@Test func secureLoggerLogsMessagesAsynchronously() async throws
{
	...
	await withCheckedContinuation { continuation in
		...
		let expected1 = [
			...
			"Hello, world!"
		].joined(separator: log.configuration.columnSeparator)
		...
		#expect(!log.needsFlushing)
		
		log.log(type: .info, "Hello, world!", now: now1)
		log.log(type: .error, "Goodbye, world!", now: now2)
		
		#expect(log.needsFlushing)
		
		print("[TEST] Sleeping...")
		
		usleep(UInt32(log.configuration.flushInterval * 1_000_000 + 0.5))
		
		print("[TEST] Sleep complete")
		#expect(!log.needsFlushing)
		
		let lines = ((try? log.secureFile.readAsString()) ?? "").split(separator: "\n")
